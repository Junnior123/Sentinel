...
class Program {
 [STAThread]static void Main(string[] args){var root=Path.GetFullPath(args[0]);var app=new Watchblock.App.App();app.InitializeComponent();SynchronizationContext.SetSynchronizationContext(new DispatcherSynchronizationContext());var window=new Watchblock.App.MainWindow();window.Show();_ = Dispatcher.CurrentDispatcher.InvokeAsync(async()=>{try{
  ...
  ((ListBox)window.FindName("Roots")).Items.Clear();((ListBox)window.FindName("Roots")).Items.Add(Path.Combine(root,"samples/private"));if(((Button)window.FindName("ScanButton")).IsEnabled)throw new Exception("Scan enabled before consent");((CheckBox)window.FindName("Consent")).IsChecked=true;await Task.Delay(350);Capture(window,Path.Combine(root,"artifacts/app-consent.png"));((Button)window.FindName("ScanButton")).RaiseEvent(new RoutedEventArgs(Button.ClickEvent));await Wait(()=>((Button)window.FindName("SubmitButton")).IsEnabled,60);
  if(((DataGrid)window.FindName("Files")).Items.Count!=11)throw new Exception("Expected 11 sample files: "+((TextBlock)window.FindName("Status")).Text);
  ((Button)window.FindName("SubmitButton")).RaiseEvent(new RoutedEventArgs(Button.ClickEvent));await Wait(()=>((Button)window.FindName("DeleteButton")).IsEnabled,60);
  ...
  File.WriteAllText(Path.Combine(root,"artifacts/app-e2e.json"),read);Console.WriteLine("PASS WPF invitation → consent → scan 11 real fixtures → submit → API report. Screenshots saved.");
