...
public sealed class Scanner(RulePack rules) {
 ...
 private async Task InspectAsync(string path,string label,ScanScope scope,CancellationToken ct){
  string id=Guid.NewGuid().ToString();long size=0;byte[]? buffer=null;try{
   ...
   await using var source=new FileStream(path,FileMode.Open,FileAccess.Read,FileShare.Read,65536,FileOptions.Asynchronous|FileOptions.SequentialScan);
   if(size<=2*1024*1024){buffer=System.Buffers.ArrayPool<byte>.Shared.Rent(Math.Max(1,(int)size));await source.ReadExactlyAsync(buffer.AsMemory(0,(int)size),ct);}
   using var cached=buffer==null?null:new MemoryStream(buffer,0,(int)size,false);
   Stream stream=cached??(Stream)source;var h=Convert.ToHexStringLower(await SHA256.HashDataAsync(stream,ct));ct.ThrowIfCancellationRequested();stream.Position=0;
   ...
   if(magic[0]==0x50&&magic[1]==0x4b){format="zip";try{using var zip=new ZipArchive(stream,ZipArchiveMode.Read,true);long expanded=0;int count=0;foreach(var entry in zip.Entries){ct.ThrowIfCancellationRequested();if(++count>10000||entry.Length>16*1024*1024||(expanded+=entry.Length)>64*1024*1024||entry.Length>Math.Max(entry.CompressedLength,1)*500){status="partial";reason="압축 검사 한도";break;}var name=entry.FullName.Replace('\\','/');if(name.StartsWith('/')||name.Split('/').Contains("..")||name.Contains(':')){status="partial";reason="안전하지 않은 압축 경로";continue;}entries.Add(name);if(name=="pack.mcmeta")format="resource-pack";if(requestedHashes.Contains(name)){using var fingerprint=entry.Open();entryHashes[name]=Convert.ToHexStringLower(await SHA256.HashDataAsync(fingerprint,ct));}if(name.EndsWith(".jar")||name.EndsWith(".zip")){status="partial";reason="내장 압축파일은 별도 검사 필요";}
     if(name.EndsWith(".class")||name is "fabric.mod.json" or "META-INF/mods.toml" or "META-INF/neoforge.mods.toml" or "mcmod.info"){
      using var es=entry.Open();var bytes=new byte[(int)entry.Length];await es.ReadExactlyAsync(bytes,ct);var text=name.EndsWith(".class")?ClassFile.Constants(bytes,needsJavaStrings):Encoding.UTF8.GetString(bytes);if(strings.Length+text.Length<16*1024*1024)strings.Append(text);else{status="partial";reason="클래스 검사 한도";}
      if(name=="fabric.mod.json"){using var doc=JsonDocument.Parse(bytes);if(doc.RootElement.TryGetProperty("id",out var v)&&v.GetString() is string mod)mods.Add(mod);format="fabric-jar";}
  ...
  }catch(OperationCanceledException){throw;}catch(Exception e)when(e is IOException or UnauthorizedAccessException or System.Security.SecurityException){files.Add(new(id,label,size,null,e is UnauthorizedAccessException?"읽기 권한 없음":"파일 읽기 실패","unknown","error",null,[]));}finally{if(buffer!=null)System.Buffers.ArrayPool<byte>.Shared.Return(buffer,clearArray:true);}
 }
}
